error.f <- function(data, l.yhat, method, cv.error.ind = c(TRUE, FALSE),
                    R = NULL, k = NULL, B = NULL, w.folds = NULL,
                    col.y, family, weights = NULL){
  
  # if(method %in% c("JKn", "cv", "BRR", "split", "extrapolation")){
  #   delete <- weights
  # }
  ...
  for(r in 1:R){
    ...
    for(kk in 1:k){
      ...
      if(method %in% c("JKn", "cv", "BRR", "bootstrap", "subbootstrap")){
        if(w.folds == TRUE){
          yhat.name <- paste0("yhat_rw_r_",r,"_train_",kk)
        } else {
          yhat.name <- paste0("yhat_sw_r_",r,"_train_",kk)
        }
        
      }
      if(method %in% c("JKn", "cv", "BRR")){
