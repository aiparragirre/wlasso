f.plot.train.error <- function(obj.name, runs, ylim = NULL,
                               l.width = 15, l.height = 8,
                               methods, method.names,
                               cols, x.legend, y.legend){
  ...
  for(method in methods){
    for(run in runs){
      f.pop.error(res = get(obj.name), method = method, title = paste0(method.names[which(methods %in% method)],"\n\nr = ", run),
                  run = run, col = cols[which(methods %in% method)], ylim = ylim)  
    }
  }
  dev.off()
  ...
  for(run in runs){
    f.train.error(res = get(obj.name), run = run, methods = methods, method.names = method.names,
                  ylim = ylim, plot.title = paste0("Training model --> population\n\nr = ", run),
                  cols = cols, plot.type = "train.to.pop", x.legend = x.legend, y.legend = y.legend)  
  }
  for(run in runs){
    f.train.error(res = get(obj.name), run = run, methods = methods, method.names = method.names,
                  ylim = ylim, plot.title = paste0("Training model --> testing set\n\nr = ", run),
